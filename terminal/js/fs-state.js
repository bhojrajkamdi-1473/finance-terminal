...
(function () {
  ...
  function badge(status, asOf) {
    if (status === "live") return '<span class="pill pill-live">LIVE</span>';
    if (status === "delayed") {
      var t = asOf ? " " + esc(String(asOf).slice(0, 10)) : "";
      return '<span class="pill pill-delayed">DELAYED' + t + "</span>";
    }
    if (status === "unavailable") return '<span class="pill pill-na">UNAVAILABLE</span>';
    return '<span class="pill pill-bad">ERROR</span>';
  }
  /* Provenance: [badge] [class-tag] source · as_of. Never invents fields. */
  function provenance(env, cls) {
    if (!env) return "";
    var bits = [badge(env.status || env.timeliness, env.as_of)];
