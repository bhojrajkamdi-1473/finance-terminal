...
| Indian Stock Market API (`stock.indianapi.in`) | quote, profile, statements, earnings, ratings, ownership, actions, news (NSE/BSE only) | free, key-gated | `INDIAN_STOCK_MARKET_API_KEY` (X-API-Key header) | quote 5m, domains 24h | 30/min + 2000/day self-imposed guard | delayed snapshot | old documented host down; Indian symbols only |
...
`ALPHA_VANTAGE_API_KEY`, `TWELVE_DATA_API_KEY`, `INDIAN_STOCK_MARKET_API_KEY`,
...
- No keys configured in this environment → AV/TD/Indian legs honestly
  unavailable; Yahoo covers quotes/history/news/actions.
- Indian Stock Market API old host down; keyed host unverified without key.
- Stooq bot-wall/401 → US-history fallback only when CSV validates.
